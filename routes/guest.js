...
router.get('/status',auth,async(req,res)=>{if(!req.user.guest)return res.status(400).json({error:'Not a guest session'});const result=await req.app.locals.pool.query('SELECT quote_count FROM guest_sessions WHERE id=$1 AND expires_at>NOW() AND converted_user_id IS NULL',[req.user.id]);if(!result.rows.length)return res.status(401).json({error:'Guest session expired'});res.json({used:result.rows[0].quote_count,remaining:Math.max(0,3-result.rows[0].quote_count)});});
