...
(async()=>{
  ...
  try{
    ...
    const pdfPromise=page.waitForEvent('download');await page.locator('#btn-download-pdf').click();const downloaded=await pdfPromise;
    const pdfBytes=fs.readFileSync(await downloaded.path());assert.equal(pdfBytes.subarray(0,5).toString(),'%PDF-');
    const popupPromise=page.waitForEvent('popup');await page.getByRole('button',{name:/Print/}).click();const popup=await popupPromise;
    ...
    const me=await (await fetch(base+'/api/auth/me',{headers:{Authorization:'Bearer '+account.token}})).json();assert.equal(me.quotes_remaining,null);assert.equal(me.trial_active,true);
