...
async function getDailyTrafficReport({ date = yesterday(), env = process.env, fetchImpl = fetch } = {}) {
  validateDate(date);
  ...
  const [gaTotals, gaSources, gaPages, scTotals, scQueries, scPages] = await Promise.all([
    googlePost(gaUrl, { dateRanges: [range], metrics: [{ name: 'activeUsers' }, { name: 'sessions' }] }, token, fetchImpl),
    googlePost(gaUrl, { dateRanges: [range], dimensions: [{ name: 'sessionDefaultChannelGroup' }], metrics: [{ name: 'sessions' }, { name: 'activeUsers' }], orderBys: [{ metric: { metricName: 'sessions' }, desc: true }], limit: 10 }, token, fetchImpl),
    googlePost(gaUrl, { dateRanges: [range], dimensions: [{ name: 'landingPagePlusQueryString' }], metrics: [{ name: 'sessions' }, { name: 'activeUsers' }], orderBys: [{ metric: { metricName: 'sessions' }, desc: true }], limit: 10 }, token, fetchImpl),
    googlePost(scUrl, { startDate: date, endDate: date }, token, fetchImpl),
    googlePost(scUrl, { startDate: date, endDate: date, dimensions: ['query'], rowLimit: 10 }, token, fetchImpl),
    googlePost(scUrl, { startDate: date, endDate: date, dimensions: ['page'], rowLimit: 10 }, token, fetchImpl)
  ]);
  const totals = scTotals.rows?.[0] || {};
  return {
    date, generatedAt: new Date().toISOString(),
    ga4: {
      users: metric(gaTotals.rows?.[0], 0), sessions: metric(gaTotals.rows?.[0], 1),
      trafficSources: (gaSources.rows || []).map((row) => ({ source: row.dimensionValues?.[0]?.value || '(not set)', sessions: metric(row, 0), users: metric(row, 1) })),
      landingPages: (gaPages.rows || []).map((row) => ({ page: row.dimensionValues?.[0]?.value || '(not set)', sessions: metric(row, 0), users: metric(row, 1) }))
    },
    searchConsole: {
      clicks: searchMetric(totals, 0), impressions: searchMetric(totals, 1), ctr: searchMetric(totals, 2), averagePosition: searchMetric(totals, 3),
      topQueries: (scQueries.rows || []).map((row) => ({ query: row.keys?.[0] || '', clicks: searchMetric(row, 0), impressions: searchMetric(row, 1), ctr: searchMetric(row, 2), averagePosition: searchMetric(row, 3) })),
      topPages: (scPages.rows || []).map((row) => ({ page: row.keys?.[0] || '', clicks: searchMetric(row, 0), impressions: searchMetric(row, 1), ctr: searchMetric(row, 2), averagePosition: searchMetric(row, 3) }))
    }
  };
}
